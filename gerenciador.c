...
void lpa_memoriaLivre(ListaPreAlocada *lpa, size_t tamanho_, void *memBase)
{
    ...
    while(1)
    {
        ...
        else
        {
            if(lpa->livres != 0)
            {
                ...
                // Procurar por um bloco de bits não preenchido no bitmap
                while (lpa->bitmap[posAtual] == (unsigned long)-1) {
                    posAtual++;
                }
                
                // Encontrar o primeiro bit livre dentro do bloco de bits
                unsigned long int posCampo = 0;
                while (lpa->bitmap[posAtual] & (0x1UL << posCampo)) {
                    posCampo++;
                }
                
                // representa o índice do  nó disponível na lista pré-alocada
                unsigned long int pos = posAtual * (sizeof(unsigned long) * 8) + posCampo;
                
                // sinalizar que um nó a menos está disponível
                --lpa->livres; // decrementa o número de nós livres na lista pré-alocada
                
                lpa->bitmap[posAtual] |= (0x1UL << posCampo); // marca o bit correspondente ao nó alocado como ocupado no bitmap
                
                // Atribuir a posição e o ponteiro pai ao nó alocado
